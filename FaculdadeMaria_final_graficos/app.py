...
applyTheme();
    ...
    derrotas = max(total_fechadas - vencedoras,0)
    vencedoras_chart = {
        'labels':['Vencedoras','Perdedoras'],
        'values':[vencedoras, derrotas]
    }
    ativos_count = {}
    for o in ops:
        a = str(o.get('Ativo','--'))
        ativos_count[a] = ativos_count.get(a,0)+1
    ativos_chart = {
        'labels': list(ativos_count.keys())[:10],
        'values': list(ativos_count.values())[:10]
    }
    ...
    return render_template(
        ...
        hist=hist,
        brl=brl,
        vencedoras_chart=vencedoras_chart,
        ativos_chart=ativos_chart,
        pct=pct
    )
