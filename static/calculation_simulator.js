(()=>{
  ...
  const callFields=['calcQuantity','calcAveragePrice','calcSpotPrice','calcStrike','calcPremium','calcExpiry','calcCosts','calcTaxRate','calcTargetRoi'];
  const putFields=['putCalcCurrentQuantity','putCalcCurrentAverage','putCalcSpot','putCalcContracts','putCalcContractSize','putCalcStrike','putCalcPremium','putCalcExpiry','putCalcCosts','putCalcTaxRate'];
  ...
  const daysToExpiry=value=>{
    ...
    return Math.ceil((Date.UTC(year,month-1,day)-Date.UTC(Number(current.year),Number(current.month)-1,Number(current.day)))/86400000);
  };
  const expiryLabel=value=>{const days=daysToExpiry(value);return days===null?'--':days<0?`Vencida há ${Math.abs(days)} dia(s)`:days===0?'Vence hoje':`${days} dia(s)`};
  const calculateCall=()=>{
    ...
    const premiumGross=premium*quantity-costs,premiumTax=Math.max(premiumGross,0)*rate,netPremium=premiumGross-premiumTax,adjustedAverage=quantity?average-netPremium/quantity:0,operationCapital=strike*quantity,operationRoi=operationCapital?netPremium/operationCapital*100:0;
    $('calcNetRoi').textContent=percent(roi);$('calcOperationRoi').textContent=percent(operationRoi);$('calcDaysToExpiry').textContent=expiryLabel($('calcExpiry').value);$('calcNetProfit').textContent=money(netExercise);$('calcTax').textContent=money(taxExercise);$('calcEffectiveExit').textContent=money(effectiveExit);$('calcNetPremium').textContent=money(netPremium);$('calcAdjustedAverage').textContent=money(adjustedAverage);$('calcStrikeDistance').textContent=spot&&strike?percent((strike/spot-1)*100):'--';
  ...
  const calculatePut=()=>{
    ...
    const assignedQuantity=contracts*contractSize,grossPremium=premium*assignedQuantity-costs,tax=Math.max(grossPremium,0)*rate,netPremium=grossPremium-tax,premiumPerAssignedShare=assignedQuantity?netPremium/assignedQuantity:0,requiredCapital=strike*assignedQuantity,operationRoi=requiredCapital?netPremium/requiredCapital*100:0,newLotAverage=Math.max(strike-premiumPerAssignedShare,0),combinedQuantity=currentQuantity+assignedQuantity,consolidatedAverage=combinedQuantity?((currentAverage*currentQuantity)+requiredCapital-netPremium)/combinedQuantity:newLotAverage,expiredAverage=currentQuantity?Math.max(currentAverage-netPremium/currentQuantity,0):null;
    $('putCalcOperationRoi').textContent=percent(operationRoi);$('putCalcDaysToExpiry').textContent=expiryLabel($('putCalcExpiry').value);$('putCalcAssignedQuantity').textContent=assignedQuantity.toLocaleString('pt-BR');$('putCalcRequiredCapital').textContent=money(requiredCapital);$('putCalcNetPremium').textContent=money(netPremium);$('putCalcNewLotAverage').textContent=money(newLotAverage);$('putCalcPremiumDiscount').textContent=`${money(premiumPerAssignedShare)}/ação`;$('putCalcConsolidatedAverage').textContent=money(consolidatedAverage);$('putCalcAverageAfterAssignment').textContent=money(consolidatedAverage);$('putCalcAverageIfExpired').textContent=expiredAverage===null?'Não possui ações':money(expiredAverage);$('putCalcStrikeDistance').textContent=spot&&strike?percent((strike/spot-1)*100):'--';
