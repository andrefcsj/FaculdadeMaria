...
def metrics(ops: List[Dict[str, object]], fechadas: List[Dict[str, str]], cfg: Dict[str, float]) -> Dict[str, float | str | int]:
    ...
    return {"capital_total": capital_total, "capital_comp": capital_comp, "caixa": max(capital_total - capital_comp, 0), "premios_ativos": premios_ativos, "lucro_mes": lucro_mes, "darf": darf, "roi_mes": roi_mes, "roi_abertas": roi_abertas, "abertas": len(abertas), "encerradas": len(ops) - len(abertas)}
