...
def op_fechadas():
    ...
    meses = sorted({str(o.get('Mes_abertura','')) for o in fechadas if o.get('Mes_abertura')})
    options = '<option value="">Todos os meses</option>' + ''.join(
        f'<option>{m}</option>' for m in meses
    )
    ...
    for o in fechadas:
        ...
        linhas.append(
            ...
            f"<a href='/excluir/{oid}' title='Excluir' onclick=\"return confirm('Excluir esta operação?')\">×</a>"
            f"</td></tr>"
        )

    tabela = ''.join(linhas) if linhas else "<tr><td colspan='10'>Nenhuma operação fechada.</td></tr>"

    html = f'''<!doctype html>
    <html lang="pt-BR"><head><meta charset="utf-8">
    <title>Cortex Invest PRO v2.9.3 - Operações Fechadas</title>
    <style>{CSS}</style></head>
    <body>
    <main style="margin-left:0;padding:25px">
      <h1>Operações <span>Fechadas</span></h1>

      <p><select style="max-width:260px">{options}</select></p>
      ...
      <section class="metrics">
        ...
        {metric_card('🎯','ROI MÉDIO',pct((sum(float(o.get("ROI",0)) for o in fechadas)/len(fechadas)) if fechadas else 0),'Operações fechadas','cyan')}
        {metric_card('📁','OP. FECHADAS',str(len(fechadas)),'Encerradas','blue')}
      </section>

      <section class="grid four" style="margin-top:15px">
        <div class="panel"><h2>Velocímetro ROI</h2>{gauge((sum(float(o.get("ROI",0)) for o in fechadas)/len(fechadas)) if fechadas else 0)}</div>
        <div class="panel"><h2>ROI Mensal</h2><p>Ligação de gráfico na próxima etapa.</p></div>
      ...
      <section class="panel" style="margin-top:15px">
        ...
        <table class="ops">
        <thead><tr><th>Ação</th><th>Opção</th><th>Strike</th><th>Tipo</th><th>Contratos</th><th>Prêmio</th><th>Capital</th><th>Data</th><th>ROI</th><th>Ações</th></tr></thead>
        <tbody>{tabela}</tbody>
        </table>
      </section>
