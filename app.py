...
def index():
    ...
    <aside><div class="logo"><div class="brain">✺</div><div class="brand">CORTEX<br><span>INVEST</span></div></div><div class="strategy">WHEEL STRATEGY</div><div class="side-block">📅<div><b>DATA ATUALIZAÇÃO</b><br>{datetime.now().strftime("%d/%m/%Y<br>%H:%M:%S")}</div></div><label>MÊS SELECIONADO</label><select><option>{ind["mes_atual"]}</option></select><nav><a href='/' class="active">🏠 Dashboard</a><a href='/operacoes-abertas'>📂 Operações Abertas</a><a href='/op-fechadas'>✅ Operações Fechadas</a><a href='/historico'>📊 Histórico</a><a href='/desempenho'>📈 Desempenho</a><a href='/carteira'>💼 Carteira</a><a href='/relatorios'>📄 Relatórios</a><a href='/configuracoes'>⚙️ Configurações</a><a href='/backup'>💾 Backup</a></nav><div class='theme-box'><div class='theme-label'>TEMA</div><div class='theme-toggle'><div class='theme-icon'>☀️</div><div class='theme-switch' onclick='toggleTheme()'></div></div></div><div class="quote">“A consistência é o que transforma estratégia em patrimônio.”<br><small>– CORTEX INVEST</small></div><div class="version">VERSÃO 3.3</div></aside>
