...
def metrics(ops: List[Dict[str, object]], fechadas: List[Dict[str, str]], cfg: Dict[str, float]) -> Dict[str, float | str | int]:
    ...
    return {"capital_total": capital_total, "capital_comp": capital_comp, "caixa": max(capital_total - capital_comp, 0), "premios_ativos": premios_ativos, "lucro_mes": lucro_mes, "darf": darf, "roi_mes": roi_mes, "roi_abertas": roi_abertas, "roi_medio_abertas": roi_medio_abertas, "roi_medio_fechadas": roi_medio_fechadas, "mes_atual": mes_atual, "abertas": len(abertas), "encerradas": len(ops) - len(abertas)}
...
def gauge(value: float, titulo: str = "ROI") -> str:
    ...
    <svg viewBox="0 0 260 170">
      ...
      <text x="72" y="48" fill="#e7f0ff" text-anchor="middle" font-size="13">1,5%</text>
